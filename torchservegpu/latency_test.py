...
async def send_request(client, image_data):
    ...
    files = {"data": ("happy.jpg", image_data, "image/jpeg")}
    try:
...
def main():
    ...
    # Run the simulation with 5000 concurrent requests
    latencies = asyncio.run(simulate_requests(image_data, total_requests=10000, concurrent_requests=500))
