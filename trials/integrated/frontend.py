...
def mixer_callback(frames: List[av.VideoFrame]) -> av.VideoFrame:
    ...
    predictions = []  # Store predictions for all participants

    for i in range(n_inputs):
        frame = frames[i]
        if frame is None:
            predictions.append("No Frame")
            continue
        ...
        resized_img = cv2.resize(img, (window_w, window_h))
        cv2.putText(
            resized_img,
            emotion,
            (10, 30),
        ...
        buffer[window_y0:window_y1, window_x0:window_x1, :] = resized_img
